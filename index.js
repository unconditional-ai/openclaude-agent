...
async function runAgent(transcript, slack_context = null, attachments = [], threadTs = null, resumeFromPause = false) {
  ...
  // code_execution_20260120 sandbox: when Claude runs a tool from inside the
  // Python sandbox (allowed_callers: ["code_execution_20260120"]), the API
  // returns a `tool_use` block with caller.type=code_execution_20260120 plus
  // a top-level `container: { id, expires_at }` on the response. The follow-up
  // request that delivers the tool_result MUST include `container: <id>` —
  // the API hard-rejects otherwise ("container_id is required when there are
  // pending tool uses generated by code execution with tools"). We capture
  // the id off every response and pass it on every subsequent request, which
  // also reuses sandbox state (files, imports) across iterations for free.
  ...
  while (iteration < maxIterations) {
    ...
    const t0 = Date.now();
    // Build the request once so the retry path (container expired) can reuse
