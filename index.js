...
async function runAgent(transcript, slack_context = null, attachments = [], threadTs = null, resumeFromPause = false) {
  ...
  // Sandbox container id. When code_execution_20260120 calls a tool
  // programmatically (caller.type === "code_execution_20260120"), the response
  // carries a `container.id` and the sandbox is paused waiting for our
  // tool_result. The next messages.create MUST pass `container: <id>` or the
  // API rejects with `container_id is required when there are pending tool
  // uses generated by code execution with tools`. Containers expire after 4.5
  // minutes of idle time, but a single /run never idles, so capturing the id
  // and passing it on every subsequent turn is enough.
  ...
  while (iteration < maxIterations) {
    ...
    // Capture the sandbox container id the first time it appears, and keep
    // resending it. The API only requires it when a programmatic tool call is
    // pending, but it's harmless to send on every subsequent turn.
